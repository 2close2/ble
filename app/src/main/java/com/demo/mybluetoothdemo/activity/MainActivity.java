...
public class MainActivity extends AppCompatActivity implements AdapterView.OnItemClickListener {
        ...
        switch (view.getId()) {
            ...
            case R.id.btn_send://发送数据按钮
                if (bleConnectUtil.isConnected()) {
                    ...
                    cmd.add("68444444563412681104343433371F16");
                    cmd.add("68444444563412681104353433372016");
                    cmd.add("68444444563412681104353733372316");
                    cmd.add("684444445634126811043E3733372C16");
                    cmd.add("68444444563412681104343433371F16");
                    cmd.add("68444444563412681104353433372016");
                    cmd.add("68444444563412681104353733372316");
                    cmd.add("684444445634126811043E3733372C16");
                    cmd.add("684444445634126811043F3733372D16");
                    cmd.add("68444444563412681104343433371F16");
                    cmd.add("68444444563412681104353433372016");
                    cmd.add("68444444563412681104353733372316");
                    cmd.add("684444445634126811043E3733372C16");
    ...
    private void sendDataByBle(final String currentSendAllOrder, String title) {
        if (currentSendAllOrder.length() < 0 || currentSendAllOrder.length() > 250) {
            Toast.makeText(MainActivity.this, "发送数据帧数据长度不对", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        Log.d("数据帧", "发送数据帧" + currentSendAllOrder);
...
//            final boolean[] isSuccess = new boolean[1];
//            //sd
//            if (currentSendAllOrder.length() <= 250) {
//                sData = CheckUtils.hex2byte(currentSendOrder);
//                mBluetoothGattCharacteristic.setValue(sData);
//                isSuccess[0] = bleConnectUtil.sendData(mBluetoothGattCharacteristic);
//                Log.d("数据帧", "发送");
